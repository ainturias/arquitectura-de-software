...
class DMateria {
    ...
    public function crear(): bool {
        $sql = "INSERT INTO materia (sigla, nombre_materia, nivel) VALUES (?, ?, ?)";
        $stmt = $this->con->prepare($sql);
        return $stmt->execute([$this->sigla, $this->nombre_materia, $this->nivel]);
    }

    public function editar(): bool {
        $sql = "UPDATE materia SET sigla=?, nombre_materia=?, nivel=? WHERE id_materia=?";
        $stmt = $this->con->prepare($sql);
        $stmt->execute([
            $this->sigla, $this->nombre_materia, $this->nivel, $this->id
        ]);
